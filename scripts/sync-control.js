#!/usr/bin/env node
// Install and control the per-user macOS launchd job for the weekly profile
...
function normalizeConfig(value = {}) {
  ...
  new Intl.DateTimeFormat('en', { timeZone: config.timeZone }).format(new Date());
...
function scheduleLabel(config) {
  return `${WEEKDAYS[config.weekday]} ${String(config.hour).padStart(2, '0')}:${String(config.minute).padStart(2, '0')}`;
}
...
async function turnOn() {
  ...
  console.log(`Weekly sync is ON (${scheduleLabel(config)}, ${config.timeZone})`);
...
function turnOff() {
  ...
  console.log('Weekly sync is OFF. The plist is kept so it can be enabled again.');
}
...
async function changeSchedule(weekday, time) {
  ...
  console.log(`Schedule saved: ${scheduleLabel(next)} (${next.timeZone})`);
}
...
async function menu() {
  ...
  try {
    ...
    if (choice === '1') await turnOn();
      ...
      const weekday = await interface_.question('星期（0=周日，1=周一…6=周六）：');
      const time = await interface_.question('时间（HH:MM）：');
      await changeSchedule(weekday, time);
    } else if (choice === '5') {
      console.log(`\n--- ${LOG_PATH} ---\n${recentLog(LOG_PATH)}`);
...
async function main() {
  ...
  if (command === 'on' || command === 'enable' || command === 'install') await turnOn();
    ...
    console.log('Usage: node scripts/sync-control.js on|off|status|run|schedule <0-6|Sun-Sat> <HH:MM>|menu');
  } else {
